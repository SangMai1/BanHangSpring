...
public interface ChucNang1Repository extends JpaRepository<ChucNang1, Long>{
	...
	@Transactional
	@Query(value = "SELECT * FROM qtht_chucnang WHERE tenchucnang @@ to_tsquery(?1) and isdelete = 0", nativeQuery = true)
	List<ChucNang1> findByTenchucnang(String tenchucnang);
	
	@Transactional
	@Query(value = "SELECT * FROM qtht_chucnang WHERE machucnang @@ to_tsquery(?1) and isdelete = 0", nativeQuery = true)
	...
	@Query(value = "WITH RECURSIVE tree AS\r\n" + 
			...
			"			LEFT JOIN qtht_nguoidungvachucnang ndvcn ON ndvcn.idchucnang = cn.id\r\n" + 
			"			LEFT JOIN qtht_nguoidung nd ON nd.id = ndvcn.idnguoidung OR  nd.id = ndvnnd.idnguoidung \r\n" + 
